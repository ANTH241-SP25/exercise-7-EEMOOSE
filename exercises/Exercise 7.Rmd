...
I COMPLETED THE TESTS FOR AGE VARIANCE BY GENDER AT THE VERY BOTTOM OF THIS EXERCISE SHEET
...
the T test supports the null hypothesis, confirming there is a true difference in means between the two Gender groups that is NOT equal to zero.
```{r}
...
#null: there is a significant difference in amount of daily sedentary living between people with and without health insurance
#alternative: people with and without health insurance do not have differing variance in daily sedentary living time
Nhanes_Sedentary <- NHANES_healthins%>%
  filter(Sedentary<1321)
#descriptive statistics of Sedentary groups by HealthIns:
Nhanes_Sedentary%>%
  drop_na(Sedentary)%>%
  group_by(HealthIns)%>%
  summarize(mean(Sedentary), sd(Sedentary), count=n())
#Test to see if the variances of the continuous variables is equal across diff levels of cat variable
...
NHANES_healthins%>%
  ...
  group_by(HealthIns)%>%
  summarize(mean(BMI), sd(BMI), count=n())
#2. Run a test (F test) to see if the variances of the continuous variable are equal across different levels of the categorical variable:
#var.test(Contvar~Catvar, data=, alternative="two.sided")
...
The above T test supports the alternative hypothesis, that BMI does vary by health insurance status, and sedentary living minutes also varies by health insurance status-- this difference in means is in fact statistically significant to an exponential degree-- the farther T i from zero, positive or negative, shows a likelihood of difference
...
#the null H is 'the amount of calories you consume will be the same across the BMI categories'
#the alternative H is that there will be a significant difference in caloric intake across BMI
...
ggplot(NHANESdata, aes(x=BMIcat, y=DR1TKCAL)) + geom_boxplot(fill="yellow") + labs(x="BMI Category", y="calories per day", title="daily calories by BMI Cat")
...
the above test does confirm the alternative hypothesis, that there will be a significant difference in caloric intake across BMI, is true between the underweight category and the other BMI cats, but to note that the other categories do not significantly differ from each other, just from the underweight.
...
#the null H is 'the ratio of family income to poverty will be the same across education levels'
#the alternative hypothesis is that there will be a significant difference in family income-to-poverty ratio across HHeducation levels
...
ggplot(NHANESdata, aes(x=HHEducation, y=RFITP)) + geom_boxplot(fill="yellow") + labs(x="HH education level", y="ratio of income to poverty index", title="poverty index by education level")
...
the above test confirms the alternative hypothesis, that there is a significant difference in ratio of family income to poverty line across head of household education, is true between each categorized level of HHeducation, on each level there is a significant difference in poverty rates. People with high school education have a significantly higher rate of poverty than those with a college education.
...
```{r}
dunn.test(x=NHANESdata$DR1TTFAT, g=NHANESdata$HHFoodSec, kw=TRUE)

```


```{r}
#what are the means, sd, and counts for Age by Gender?
NHANESdata%>%
  drop_na(Age)%>%
  group_by(Gender)%>%
  summarize(mean(Age), sd(Age), count=n())

#we want to see if male variance by age matches female variance by age-- if they are not the same, we have to note that the difference should be accounted for in tests
var.test(Age~Gender, data=NHANESdata,
         alternative=("two.sided"))
t.test(Age~Gender, var.equal=TRUE, data=NHANESdata)
```
it tells us that the true variances are similar -- the P value being higher than 0.05 means the difference between age variance were not statistically significant

P value tells us there is clearly not a significant difference in age distribution/variation

Question: Does the above T-test support the null hypothesis or the alternative hypothesis?
the T test supports the alternative hypothesis, confirming there is not a statistically significant  difference in means between the two gendered age groups by way of variance
